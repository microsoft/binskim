    ...
    public class ELFBinary : BinaryBase, IDwarfBinary
    {
        public ELFBinary(Uri uri) : base(uri)
        {
            try
            {
                ...
                List<DwarfPublicSymbol> publicSymbols = new List<DwarfPublicSymbol>();
        ...
        public ulong NormalizeAddress(ulong address)
        {
            ...
            return section != null && section.Flags.HasFlag(SectionFlags.Allocatable)
        ...
        private void LoadDwo()
        {
            ...
            DwarfSymbol skeletonOrCompileSymbol = CompilationUnits.SelectMany(c => c.Symbols)
                .FirstOrDefault(s => s.Tag == DwarfTag.SkeletonUnit || s.Tag == DwarfTag.CompileUnit);
            KeyValuePair<DwarfAttribute, DwarfAttributeValue>? dwo = skeletonOrCompileSymbol?.Attributes?.FirstOrDefault(a => a.Key == DwarfAttribute.DwoName || a.Key == DwarfAttribute.GnuDwoName);
            string dwoName = dwo?.Value?.String ?? string.Empty;
            if (!string.IsNullOrWhiteSpace(dwoName))
            {
                Uri directory = new Uri(TargetUri, ".");
                Uri dwoUri = new Uri(Path.Combine(directory.AbsolutePath, dwoName));
