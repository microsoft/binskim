...
function Build-Baselines($sourceExtension)
{
    ...
    Get-ChildItem $testsDirectory -Filter $sourceExtension | ForEach-Object {
        ...
        Write-Host "$utility analyze "$input" --output "$outputTemp" --kind 'Fail;Pass' --level Error`;Warning`;Note --insert Hashes --remove NondeterministicProperties --config default --quiet --sarif-output-version Current"
        &           $utility analyze "$input" --output "$outputTemp" --kind 'Fail;Pass' --level Error`;Warning`;Note --insert Hashes --remove NondeterministicProperties --config default --quiet --sarif-output-version Current
