    ...
    public class AnalyzeCommandTests
    {
        ...
        public void AnalyzeCommand_IgnoreExceptionInCanAnalyzeErrorTest()
        {
            ...
            var options = new AnalyzeOptions
            {
                ...
                IgnoreExceptionInCanAnalyzeError = true,
                DataToInsert = new[] { OptionallyEmittedData.Hashes }
            };
            var command = new MultithreadedAnalyzeCommand();
            command.Run(options);
            var log = SarifLog.Load(fileName);
            log.Runs[0].Invocations[0].ToolConfigurationNotifications.Count(t => t.Message.Text.Contains("The exception may have resulted from a problem related to parsing the analysis target and not specific to the rule, however.")).Should().Be(4);
        }
        ...
        public void AnalyzeCommand_NotIgnoreExceptionInCanAnalyzeErrorTest()
        {
            ...
            var options = new AnalyzeOptions
            {
                ...
                Threads = 10,
                IgnorePdbLoadError = true,
                IgnoreExceptionInCanAnalyzeError = false,
                DataToInsert = new[] { OptionallyEmittedData.Hashes }
            };
            var command = new MultithreadedAnalyzeCommand();
            command.Run(options);
