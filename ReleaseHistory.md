...
## UNRELEASED
* BUG: Fix `System.InvalidOperationException`: `Sequence contains more than one matching element` when `--trace` is provided. [914](https://github.com/microsoft/binskim/pull/914)
* BUG: Fix `--trace` missing supported values from SARIF SDK (`ScanTime`, `RuleScanTime`, `PeakWorkingSet`, `TargetsScanned`, `ResultsSummary`). [914](https://github.com/microsoft/binskim/pull/914)
